...
void __fastcall TForm1::BtnDebugClick(TObject *Sender) {
	...
	if (edt_beta3->Text != "" && edt_beta0->Text != "") {
		...
		// for (ii = 0; ii < Radar.PackRec[j].CntIZ; ii++) {
		//
		// float Time = PulseToTime(i, j, ii);
		// float TimeBack = Time + 2.0 * D_start / 3.0e8;
		// float deltaTime = TimeBack - Time;
		// float deltaDistance = currentTarget.Velocity * (deltaTime);
		// float X = (D_start / 450000.0) * sin(Pi * tmpItem->Beta / 180.0);
		// float Y = (D_start / 450000.0) * cos(Pi * tmpItem->Beta / 180.0);
		// glVertex2f(X, Y);
		// currentPulse++;
		// D_start += tmpItem->deltaDistance;
		//
		...
		// float Time = PulseToTime(i, j, ii);
		//
		// float TimeBack = tmpItem->Time + 2.0 * D_start / 3.0e8;
		// float deltaTime = tmpItem->TimeBack - tmpItem->Time;
		// float deltaDistance = currentTarget.Velocity * (tmpItem->deltaTime);
		// float X = (D_start / 450000.0) * sin(Pi * tmpItem->Beta / 180.0);
		// float Y = (D_start / 450000.0) * cos(Pi * tmpItem->Beta / 180.0);
		//
		// glVertex2f(X, Y);
