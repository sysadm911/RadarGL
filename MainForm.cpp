...
void __fastcall TForm1::BtnDebugClick(TObject *Sender) {
	...
	if (edt_beta3->Text != "" && edt_beta0->Text != "") {
		...
		Memo1->Lines->Add("Путь который проходит цель, км = " + FloatToStr(Route / 1000.0));
		Memo1->Lines->Add("");
		...
		Memo1->Lines->Add("Угол между нач. и кон. положением цели = " + FloatToStr(ugol));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("За время одного оборота цель пролетит расстояние, км = " + FloatToStr(currentTarget.Velocity * 10));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Угол teta = " + FloatToStr(180 / Pi * (acos((D_start*D_start + Route*Route - D_end*D_end) / (2*D_start*Route)))));
		...
		Memo1->Lines->Add("Кол-во оборотов антенны, кот. совершает антенна за время наблюдения цели= " + FloatToStr(RouteTime / 10));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Кол-во излученных пакетов за время наблюдения цели = " + FloatToStr((RouteTime / 10) *GetCountPacketOfRotaion()));
		Memo1->Lines->Add("");

		float number_of_DD = GetCountPulsesOfPacket() * (RouteTime / 10) * GetCountPacketOfRotaion();
		Memo1->Lines->Add("Кол-во излученных парциальных запусков (дискретов дальности) за время наблюдения цели = " + FloatToStr(number_of_DD));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Кол-во пакетов, кот. попадают на траекторию полета цели = " + FloatToStr(ugol / 2.0713));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Кол-во излученных парциальных запусков (дискретов дальности) кот. попадают на траекторию полета цели = " +
			FloatToStr(ugol / 2.0713 * GetCountPulsesOfPacket()));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Кол-во мегабайт для хранения массива = " + FloatToStr(4*ugol / 2.0713 * GetCountPulsesOfPacket()) / (1024. *1024.));
		Memo1->Lines->Add("");
		...
		Memo1->Lines->Add("Status.ullAvailPhys:  Количество оставшейся физической памяти в мегабайтах." +
			FloatToStr(Status.ullAvailPhys / (1024. *1024.)));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Status.ullAvailVirtual: 	Количество свободной виртуальной памяти в мегабайтах." +
		...
		while (Route > 0) {
			// Memo1->Lines->Add("Номер дискрета в кот. обнаруживается цель = " + FloatToStr(DistanceToDiskret(D_start)));
			// Memo1->Lines->Add("");
