...
void __fastcall TForm1::BtnDebugClick(TObject *Sender) {
	...
	if (edt_beta3->Text != "" && edt_beta0->Text != "") {
		...
		if (t2 > t1)
			Memo1->Lines->Add("Угол между нач. и кон. положением цели = " + FloatToStr(t2 - t1));
		if (t2 < t1)
			Memo1->Lines->Add("Угол между нач. и кон. положением цели = " + FloatToStr(t1 - t2));
		...
		Memo1->Lines->Add("Кол-во пакетов за время наблюдения цели = " + FloatToStr(RouteTime / 10 *GetCountPacketOfRotaion()));

		Memo1->Lines->Add("");

		float number_of_DD = GetCountPulsesOfPacket() * RouteTime / 10 * GetCountPacketOfRotaion();

		Memo1->Lines->Add("Кол-во парциальных запусков (дискретов дальности) за время наблюдения цели = " + FloatToStr(number_of_DD));
		Memo1->Lines->Add("");

		Memo1->Lines->Add("Кол-во мегабайт для хранения массива = " + FloatToStr(4*number_of_DD) / (1024. *1024.));
		Memo1->Lines->Add("");
